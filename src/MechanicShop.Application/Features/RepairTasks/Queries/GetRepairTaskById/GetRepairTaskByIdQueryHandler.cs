...
) : IRequestHandler<GetRepairTaskByIdQuery, Result<RepairTaskDto>>
{
    ...
    public async Task<Result<RepairTaskDto>> Handle(GetRepairTaskByIdQuery query, CancellationToken cancellationToken)
    {
        var repairTask = await _context.RepairTasks.AsNoTracking()
            .Include(r=>r.Parts).FirstOrDefaultAsync(r=>r.Id ==query.RepairTaskId,cancellationToken );
