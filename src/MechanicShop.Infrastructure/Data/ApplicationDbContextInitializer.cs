    ...
    RoleManager<IdentityRole> roleManager)
{
    ...
    public async Task TrySeedAsync()
    {
        ...
        if(_userManager.Users.All(u => u.Email != labor1.Email))
        {
            await _userManager.CreateAsync(labor1, labor1.Email);

            if (!string.IsNullOrWhiteSpace(laborRole.Name))
            {
                await _userManager.AddToRolesAsync(labor1, [laborRole.Name]);
            }
        }
        ...
        if (!_context.Employees.Any())
        {
            _context.Employees.AddRange([
                ...
                Employee.Create(Guid.Parse(labor1.Id), "John", "S.", Role.Labor).Value,
        ...
        if (!_context.RepairTasks.Any())
        {
            ...
            [
                ...
                RepairTask.Create(id: Guid.Parse("616aebb1-d515-4b40-8d47-8d5c0b67a313"), name: "Engine Oil Change", laborCost: 50.00m, estimatedDurationInMins: RepairDurationInMinutes.Min105, parts: [Part.Create(Guid.Parse("ec65225c-9066-4a1c-974f-f183c39fdd16"), name: "Engine Oil", cost: 25.00m, quantity: 1).Value, Part.Create(Guid.Parse("62ad80e3-2cff-41af-ab40-16fab8db8b38"), "Oil Filter", 10.00m, 1).Value]).Value,
        ...
        if (!_context.WorkOrders.Any())
        {
            ...
            string [] labors = [labor1.Id, labor02.Id, labor03.Id, labor04.Id];
            Spot [] spots = [Spot.A, Spot.B, Spot.C, Spot.D];

            DateTimeOffset startDate = DateTimeOffset.Now.Date.AddDays(1);
            ...
            while(startDate < endDate)
            {
                foreach (Spot spot in spots)
                {
                    ...
                    {
                        var selectedTask =
                            _context.RepairTasks.AsEnumerable().
                            DistinctBy(R => R.Id).OrderBy(_ => Guid.NewGuid())
                            .Take(Random.Shared.Next(1, Math.Min(4, repairTasks.Select(R => R.Id).Distinct().Count())))
                            .ToList();

                        var labor = labors[random.Next(labors.Length)];
                        var duration = selectedTask.Sum(R => (int)R.EstimatedDurationInMins);
                        ...
                        currentTime =
                        startDate.Add(openTime)
                        .AddMinutes(occupiedMinutes);
                    }
            ...
            var repairTaskForFirstOrder = _context.RepairTasks
                .OrderBy(_ => Guid.NewGuid())
                .Take(2)
                .ToList();
            ...
            var workOrderStartingNow = WorkOrder.Create(
                ...
                startTimeFirstOrder,
                startTimeFirstOrder.AddMinutes(repairTaskForFirstOrder.Sum(r => (int)r.EstimatedDurationInMins)),
                Guid.Parse(labor1.Id),
                Spot.A,
                repairTaskForFirstOrder).Value;

            workOrderStartingNow.UpdateState(WorkOrderState.InProgress);

            var repairTaskEndingNow = _context.RepairTasks
                .First(r => r.EstimatedDurationInMins == RepairDurationInMinutes.Min60);

            var startedAgo = utcNow.AddMinutes(-45);

            var roundStart = new DateTimeOffset(
                startedAgo.Year,
                startedAgo.Month,
                startedAgo.Day,
                startedAgo.Hour,
            ...
            var endTimeSecondOrder =
                roundStart.AddMinutes((int)repairTaskEndingNow.EstimatedDurationInMins);

            var value = WorkOrder.Create(
                Guid.NewGuid(),
                _context.Vehicles.OrderBy(_ => Guid.NewGuid()).First().Id,
                roundStart,
                endTimeSecondOrder,
